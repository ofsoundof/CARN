...
echo "$@"
source ~/virtual_enviroment/gpu-tensorflow-1.9/bin/activate
...
<<'COMMENT'
shift 2
python -u hashnets.py "$@" --sge_gpu $SGE_GPU --test_dir "$TEST_DIR/Set5" --test_procedure 1 --test_score_compute --test_runtime_compute
echo 'Testing for Set5 finished!'
echo
python -u hashnets.py "$@" --sge_gpu $SGE_GPU --test_dir "$TEST_DIR/Set14" --test_procedure 1 --test_score_compute --test_runtime_compute
